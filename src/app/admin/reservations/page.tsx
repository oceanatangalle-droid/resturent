  ...
  return (
    <div>
      <AdminPageHeader
        ...
        action={
          <button
            ...
            onClick={load}
            disabled={loading}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium text-slate-700 hover:text-slate-900 bg-white hover:bg-slate-50 border border-slate-300 transition-colors disabled:opacity-50"
          >
            <IconRefresh />
      ...
      {loading && list.length === 0 ? (
        ...
        <div className="admin-surface p-12 text-center">
          <IconCalendarEmpty />
          <p className="text-slate-700 font-medium">No reservations yet</p>
          <p className="text-slate-500 text-sm mt-1">New bookings from the Book a Table form will appear here.</p>
        </div>
      ) : (
            ...
            <table className="w-full text-left min-w-[720px]">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50">
                  ...
                  <th className="px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Guests</th>
                  <th className="px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Status</th>
                  <th className="px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Actions</th>
                  <th className="px-5 py-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Requests</th>
                </tr>
              </thead>
              <tbody>
                {sorted.map((r) => (
                  <tr key={r.id} className="border-b border-slate-200/80 last:border-0 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-slate-600 text-sm">
                      <a href={`mailto:${r.email}`} className="text-primary-600 hover:text-primary-700 hover:underline">
                        {r.email}
                      </a>
                      <div className="mt-0.5">{r.phone}</div>
                    </td>
                    <td className="px-5 py-3.5 text-slate-900">{r.guests}</td>
                    ...
                    <td className="px-5 py-3.5 text-slate-600 text-sm max-w-[180px] truncate" title={r.specialRequests || undefined}>
                      {r.specialRequests || '—'}
                    </td>
                  </tr>
                ))}
